...
struct SingleText {
	int usedLines;		// # text row used (max 4)
	const char *l[4];	// array of string pointers (the 4 row)
	int start;			// index to the firs char of the buffer text
	int len;			// text's len
};

	struct CharData {
		// Coordinates of the position in the font texture
		int x;
		int y;
		// Dimension of chars in the font texture.
		int width;
		int height;
		// Offset of the chars position
		int xoffset;
		int yoffset;
		// Space between chars
		int xadvance;
	...
	struct FontDef {
		int lineHeight;
		std::vector<CharData> P; // data for each font char
	};
	
	// vector of fonts definition 
...
struct TextVertex {
	glm::vec2 pos;		// position of the vertex of the text rendering
	glm::vec2 texCoord; // coordinate to map the font texture on the text
};


struct TextMaker {
	VertexDescriptor VD; // Vertex descriptor == how the data is passed to the GPU
	BaseProject *BP; // Pointer to the main project
	DescriptorSetLayout DSL; // Passes textures and data to the shader
	Pipeline P;
	Model M;
	Texture T;
	DescriptorSet DS;

	// Pointer to a vector of texts
	std::vector<SingleText> *Texts;

	// Initializes the TextMaker object with the project and texts to display
	void init(BaseProject *_BP, std::vector<SingleText> *_Texts) {
		BP = _BP;
		Texts = _Texts;
		// Creates the descriptor layout and the vertex format
		createTextDescriptorSetAndVertexLayout();
		// Creates the pipeline for text rendering
		createTextPipeline();
		// Creates the model and loads the font texture
		createTextModelAndTexture();

		// Increases the number of textures and descriptor sets in the pool
		BP->DPSZs.texturesInPool += 1;
		BP->DPSZs.setsInPool += 1;
	}

	// Creates the descriptor layout and sets the vertex format (TextVertex)
	void createTextDescriptorSetAndVertexLayout() {
		...
		// Initializes the descriptor layout for passing the texture to the fragment shader
		DSL.init(BP,
	...
	void createTextModelAndTexture() {
		createTextMesh(); // Creates the mesh for the text
		M.initMesh(BP, &VD); // Initializes the model with the vertex format
		T.init(BP, "textures/Fonts.png"); // Loads the font texture
	}
	
	// Creates the mesh for the text, based on the provided strings
	void createTextMesh() {
		int totLen = 0;
		// Counts the total number of characters in the texts
		for(auto& Txt : *Texts) {
		...
		// Resizes the index buffer based on the number of characters
		...
		// Character limits and texture dimensions
		int minChar = 32;
		int maxChar = 127;
		int texW = 1024;
		int texH = 512;
		
		// Initial coordinates for the text
		int tpx = 0;
		int tpy = 0;
		
		// Indices for managing the mesh
		int ib = 0, k = 0;

		// For each text...
		for(auto& Txt : *Texts) {
			Txt.start = ib;
			for(int i = 0; i < Txt.usedLines; i++) { // For each line of text
				for(int j = 0; j < strlen(Txt.l[i]); j++) { // For each character in the line
					int c = ((int)Txt.l[i][j]) - minChar; // Get the character
					if((c >= 0) && (c <= maxChar)) { // If the character is valid
						CharData d = Fonts[FontId].P[c]; // Get the character data
					
						// Create vertices for the character (4 vertices == a square)
						...
						// Define the indices for the two triangles that make up the character square
						...
						// Update the indices and horizontal position
						ib += 6;
						tpx += d.xadvance;
						k++;
					}
				}
				tpy += Fonts[FontId].lineHeight; // Move to the next line
				tpx = 0; // Reset the horizontal position
			}
			tpx = 0;
			tpy = 0;
			Txt.len = ib - Txt.start; // Update the text length
		}
	
		std::cout << "[Text] ";
	}

	// Creates the descriptor sets for the text texture
	void createTextDescriptorSets() {
		DS.init(BP, &DSL, {&T});
	}

	// Initializes pipelines and descriptor sets
	void pipelinesAndDescriptorSetsInit() {
		P.create();
		createTextDescriptorSets();
	}
	
	// Cleans up pipelines and descriptor sets
	...
	// Populates the command buffer with the text rendering commands
    void populateCommandBuffer(VkCommandBuffer commandBuffer, int currentImage, int curText = 0) {
    	P.bind(commandBuffer); // Bind the pipeline
		M.bind(commandBuffer); // Bind the model
		DS.bind(commandBuffer, P, 0, currentImage); // Bind the descriptor set
		
		// Draw the indexed vertices for the current text
